...
export async function POST() {
  try {
    ...
    console.log('🔧 [Fix Mobile Triggers Constraint] Applying corrected fixes...');
    ...
    const sqlStatements = [
      ...
      // Create new delivery notification function
...
$$ LANGUAGE plpgsql;`,
    ...
    EXECUTE FUNCTION mobile_notify_new_delivery();`,
      ...
      // Create delivery status function
...
$$ LANGUAGE plpgsql;`,
      ...
      // Create delivery status trigger
...
$$ LANGUAGE plpgsql;`,
    ...
    for (const [index, sql] of sqlStatements.entries()) {
      console.log(`🔧 Executing SQL statement ${index + 1}/${sqlStatements.length}...`);
      
      const { error } = await supabase.from('_dummy_').select('*').limit(0);
      
      // Use the Supabase SQL editor functionality
      const { error: sqlError } = await supabase.from('pg_stat_activity').select('*').limit(0);
      
      try {
        // Try direct SQL execution
        const { error: directError } = await supabase.rpc('sql', { query: sql });
        if (directError) {
          console.log(`❌ Error in statement ${index + 1}:`, directError.message);
          errorCount++;
          results.push({ index: index + 1, error: directError.message });
        } else {
          console.log(`✅ Statement ${index + 1} executed successfully`);
          results.push({ index: index + 1, success: true });
        }
      } catch (err) {
        console.log(`❌ Exception in statement ${index + 1}:`, err);
        errorCount++;
        results.push({ index: index + 1, error: err instanceof Error ? err.message : 'Unknown error' });
      }
    }

    if (errorCount === 0) {
      console.log('✅ All mobile triggers constraint fixes applied successfully');
      return NextResponse.json({
        success: true,
        message: 'All mobile triggers constraint fixes applied successfully - delivery creation should now work!',
        results,
        changes: [
          'Fixed notification_type from "delivery" to "general" (valid constraint)',
          'Fixed column ambiguity issues with explicit table aliases',
          'Maintained all existing functionality and notification content'
        ]
      });
    } else {
      console.log(`⚠️ ${errorCount} errors occurred during mobile triggers fix`);
      return NextResponse.json({
        success: false,
        message: `${errorCount} errors occurred during mobile triggers fix`,
        results,
        errorCount
      });
    }
    ...
    console.error('❌ Mobile triggers constraint fix error:', error);
    return NextResponse.json({
      success: false,
